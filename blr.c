...
static BlrKey posix_next_key(void) {
    ...
    int c = blr_read_byte();
    if (c < 0) {
        k.type = BLR_KEY_EOF_OR_ERROR;
        return k;
    }

    switch (c) {
    case 1:
        k.type = BLR_KEY_CTRL_A;
        return k;
    case 2:
        k.type = BLR_KEY_CTRL_B;
        return k;
    case 3:
        k.type = BLR_KEY_CTRL_C;
        return k;
    case 4:
        k.type = BLR_KEY_CTRL_D;
        return k;
    case 5:
        k.type = BLR_KEY_CTRL_E;
        return k;
    case 6:
        k.type = BLR_KEY_CTRL_F;
        return k;
    case 8:
        k.type = BLR_KEY_BACKSPACE;
        return k;
    case 10: /* fall-through */
    case 13:
        k.type = BLR_KEY_ENTER;
        return k;
    case 11:
        k.type = BLR_KEY_CTRL_K;
        return k;
    case 14:
        k.type = BLR_KEY_CTRL_N;
        return k;
    case 16:
        k.type = BLR_KEY_CTRL_P;
        return k;
    case 21:
        k.type = BLR_KEY_CTRL_U;
        return k;
    case 23:
        k.type = BLR_KEY_CTRL_W;
        return k;
    case 127:
        k.type = BLR_KEY_BACKSPACE;
        return k;
        ...
        if (seq0 == '[') {
            ...
            case '1': {
                ...
                if (s2 == ';') {
                    ...
                    if (s4 == 'C') {
                        k.type = BLR_KEY_WORD_RIGHT;
                        return k;
                    }
                    if (s4 == 'D') {
                        k.type = BLR_KEY_WORD_LEFT;
                        return k;
                    }
                }
                k.type = BLR_KEY_IGNORE;
...
static int blr_term_raw(BlrTermState *saved) {
    ...
    if (blr_hin == INVALID_HANDLE_VALUE || blr_hout == INVALID_HANDLE_VALUE)
        return -1;
    if (!GetConsoleMode(blr_hin, saved))
        return -1;
    DWORD raw = *saved & ~(DWORD)(ENABLE_LINE_INPUT | ENABLE_ECHO_INPUT |
    ...
    if (GetConsoleMode(blr_hout, &out_mode))
        SetConsoleMode(blr_hout, out_mode | ENABLE_VIRTUAL_TERMINAL_PROCESSING |
...
static void blr_term_restore(const BlrTermState *saved) {
    if (blr_hin != INVALID_HANDLE_VALUE)
        SetConsoleMode(blr_hin, *saved);
}
...
static BlrKey win_map_key(WORD vk, CHAR ch, DWORD ctrl) {
    ...
    case VK_DELETE:
        ...
        return k;
    case VK_LEFT:
        k.type = ctrl_pressed ? BLR_KEY_WORD_LEFT : BLR_KEY_LEFT;
        return k;
    case VK_RIGHT:
        k.type = ctrl_pressed ? BLR_KEY_WORD_RIGHT : BLR_KEY_RIGHT;
        return k;
    case VK_UP:
        k.type = BLR_KEY_UP;
        return k;
...
/* Per-call state shared by blr_read and blr_rec_read; hist is NULL for the
 * former. */
typedef struct {
    const char *prompt;
    LineBuf lb;
...
/* Move through history by dir (-1 = older, +1 = newer); saves the live line on
 * first nav. */
static void blr_nav_history(EditState *es, int dir) {
    if (!es->hist || es->hist->count == 0)
        return;
    int new_pos = es->hist_pos + dir;
    if (new_pos < 0)
        new_pos = 0;
    if (new_pos > (int)es->hist->count)
        new_pos = (int)es->hist->count;
    if (new_pos == es->hist_pos)
        return;
    if (es->hist_pos == (int)es->hist->count)
        memcpy(es->saved, es->lb.buf, es->lb.len + 1);
    es->hist_pos = new_pos;
    lb_set(&es->lb, new_pos == (int)es->hist->count
                        ? es->saved
                        : es->hist->entries[new_pos]);
}

/* Main event loop: raw-mode editing on TTY, fgets fallback otherwise. Returns
 * heap string or NULL. */
static char *blr_edit_loop(EditState *es) {
    BlrTermState saved_term;
    int raw_ok = 0;
    if (blr_is_tty())
        raw_ok = (blr_term_raw(&saved_term) == 0);
    ...
    if (!raw_ok) {
        ...
        if (!line)
            return NULL;
        if (!fgets(line, BLR_MAX_LINE, stdin)) {
            free(line);
            return NULL;
        }
    ...
    for (;;) {
        ...
        case BLR_KEY_ENTER:
            ...
            if (result)
                memcpy(result, es->lb.buf, es->lb.len + 1);
            goto done;
        case BLR_KEY_CTRL_C:
            fwrite("\r\n", 1, 2, stdout);
            fflush(stdout);
        ...
        case BLR_KEY_RIGHT:
        case BLR_KEY_CTRL_F:
            if (es->lb.pos < es->lb.len)
                es->lb.pos++;
            break;
        case BLR_KEY_WORD_LEFT:
            lb_word_left(&es->lb);
            break;
        case BLR_KEY_WORD_RIGHT:
            lb_word_right(&es->lb);
            break;
        case BLR_KEY_HOME:
        case BLR_KEY_CTRL_A:
...
char *blr_rec_read(const char *prompt, BlrHistory *hist) {
    ...
    if (line && line[0] != '\0' && hist)
        blr_history_add(hist, line);
    return line;
}

int blr_history_init(BlrHistory *hist, size_t max_entries) {
    if (!hist || max_entries == 0) {
        errno = EINVAL;
        return -1;
    }
...
int blr_history_load(BlrHistory *hist, const char *path) {
    ...
    while (fgets(line, (int)sizeof(line), f)) {
        ...
        if (n == 0)
            continue;
        if (blr_history_add(hist, line) == -1) {
            fclose(f);
            return -1;
        }
    }
