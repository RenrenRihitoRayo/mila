...
/* Allocate entries array for max_entries lines. Must be called first. Returns
 * 0/-1. */
...
/* Append a copy of line; evicts oldest on overflow. Empty strings are ignored.
 * Returns 0/-1. */
int blr_history_add(BlrHistory *hist, const char *line);

/* Write history to path (one entry per line, file is created/truncated).
 * Returns 0/-1. */
int blr_history_save(const BlrHistory *hist, const char *path);

/* Load entries from path into hist. Missing file is treated as empty (not an
   error). Returns 0/-1. Lines longer than BLR_MAX_LINE are silently truncated.
 */
int blr_history_load(BlrHistory *hist, const char *path);

/* Normal read: show prompt, edit a line, return heap string the caller must
   free(). No history nav. Returns NULL on EOF (Ctrl-D on empty line) or Ctrl-C.
   Trailing newline is stripped. */
char *blr_read(const char *prompt);

/* Recorded read: like blr_read() but Up/Down history nav is active and
   confirmed lines are saved to hist. Returns NULL on EOF or Ctrl-C. Non-empty
   lines are appended to hist before being returned. */
char *blr_rec_read(const char *prompt, BlrHistory *hist);

/* Maximum bytes per input line (override before including or via
 * -DBLR_MAX_LINE=N). */
#ifndef BLR_MAX_LINE
