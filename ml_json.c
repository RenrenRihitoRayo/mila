...
void ascii_to_unicode(char** buffer, const char* input) {
    ...
    for (size_t i = 0; i < strlen(input); ++i) {
        ...
        if (byte < 0x80) {
            switch (byte) {
                case '\b': malloc_sprintf(buffer, "\\b"); break;
    ...
    if (is_ident_start(c)) {
        ...
        if (parse_fn && strcmp(id, "fn") == 0) {
            ...
            if (match_char(s, ':')) {
                ...
                for (int i = 0; names[i]; ++i) mila_free(names[i]);
                mila_free(names);
            }
...
Value* parse_json(Src* s) {
    return parse_expr_unified(s, 0);
}

Value* parse_mjson(Src* s) {
    return parse_expr_unified(s, 1);
}
        ...
        case T_OWNED_OPAQUE: {
            if (v->type_name && strcmp(v->type_name, MILA_LPREFIX "list") == 0) {
                ...
                for (size_t i = 0; i < list->size; ++i) {
                    char* item_json = _mila_to_json_unified(ll_get(list, i), level+1, include_fn);
                ...
                for (size_t i = 0; i < dict->capacity; ++i) {
                    for (DictEntry* entry = dict->buckets[i]; entry; entry = entry->next) {
                        if (!first) malloc_sprintf(&result, ",\n");
                        first = 0;
                        char* val_json = _mila_to_json_unified(entry->value, level+1, include_fn);
                        malloc_sprintf(&result, "%*s%s: %s", level*2, "", entry->key, val_json);
                        mila_free(val_json);
                    }
                }
                malloc_sprintf(&result, "\n%*s}", (level-1)*2, "");
        ...
        case T_FUNCTION: {
            if (include_fn) {
                ...
                for (int i = 0; fn->params[i]; ++i) {
                    ...
                    if (fn->params[i+1]) malloc_sprintf(&args, "%s,", args);
                }
        ...
        case T_OWNED_OPAQUE: {
            if (v->type_name && strcmp(v->type_name, MILA_LPREFIX "list") == 0) {
                ...
                for (size_t i = 0; i < list->size; ++i) {
                    ...
                    result += _io_mila_to_json_unified(file, ll_get(list, i), level+1, include_fn);
                    if (i < list->size - 1) result += fprintf(file, ",\n");
                }
                result += fprintf(file, "\n%*s]", (level-1)*2, "");
            } else if (v->type_name && strcmp(v->type_name, MILA_LPREFIX "dict") == 0) {
                Dict* dict = (Dict*)GET_OPAQUE(v);
                ...
                for (size_t i = 0; i < dict->capacity; ++i) {
                    for (DictEntry* entry = dict->buckets[i]; entry; entry = entry->next) {
                        if (!first) result += fprintf(file, ",\n");
                        first = 0;
                        result += fprintf(file, "%*s%s: ", level*2, "", entry->key);
                        result += _io_mila_to_json_unified(file, entry->value, level+1, include_fn);
                    }
                }
                result += fprintf(file, "\n%*s}", (level-1)*2, "");
            } else {
            ...
            if (include_fn) {
                ...
                for (int i = 0; fn->params[i]; ++i) {
                    result += fprintf(file, "%s%s", fn->params[i], 
                                    fn->defaults[i] ? fn->defaults[i] : "");
                    if (fn->params[i+1]) result += fprintf(file, ",");
                }
...
char* mila_to_json(Value* v) {
    return _mila_to_json_unified(v, 1, 0);
}

char* mila_to_mjson(Value* v) {
    return _mila_to_json_unified(v, 1, 1);
}
