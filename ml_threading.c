...
static void thread_registry_init(void) {
    ...
    if (!initialized) {
        ...
        thread_registry.threads =
            mila_malloc(sizeof(ThreadContext *) * thread_registry.capacity);
        initialized = 1;
    }
}
...
static void thread_registry_expand(void) {
    ...
    ThreadContext **new_threads =
        mila_malloc(sizeof(ThreadContext *) * new_capacity);
    memcpy(new_threads, thread_registry.threads,
           sizeof(ThreadContext *) * thread_registry.count);
...
Value *native_thread_create(Env *env, int argc, Value **argv) {
    ...
    if (argv[0]->type != T_FUNCTION ||
        ...
        return verror(
            "thread.make(code, on_kill?): Requires function argument");
    }
...
Value *native_thread_join(Env *env, int argc, Value **argv) {
    ...
    if (ctx->on_kill) {
        ...
        val_release(call_function_with(frame, ctx->on_kill,
                                       vint(ctx->public_thread_id),
                                       vstring_dup("normal"), NULL));
...
void mila_threads_cleanup(void) {
    ...
    for (MThreadID i = 0; i < thread_registry.count; i++) {
        ...
        /* Daemon threads: execute cleanup handler on halt */
            ...
            ctx->is_cancelled = 1;
            Env *frame = env_new(NULL);
            val_release(call_function_with(
                frame, ctx->on_kill, vint(ctx->public_thread_id),
                vstring_dup(ctx->status == 2 ? "normal" : "interpreter halt"),
                NULL));
            val_release(ctx->on_kill);
            env_free(frame);
            ctx->on_kill = NULL;
