...
class MTagsParser:
    ...
    def _parse_line(self, line: str) -> None:
        ...
        try:
            ...
            stmt_type = parts[2]
            type_val = parts[3].strip('"')
            name = parts[4]
            decl = parts[5] if len(parts) > 5 else ""
...
def handle_message(msg):
    ...
    if method == "initialize":
        ...
        log_message(f"Completion: prefix='{prefix}', returning {len(items)} items")
        ...
        if uri in open_docs:
            ...
            if line < len(lines):
                ...
                if word in parser.symbols:
                    sym = parser.symbols[word][0]
                    server.send_response(msg_id, {
                        "uri": f"file://{Path(sym['file']).resolve()}",
                        "range": {
